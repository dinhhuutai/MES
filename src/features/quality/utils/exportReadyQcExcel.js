...
const COLS = [
  ...
  { h: 'Khuôn', w: 8, key: (r) => (r.khuon_done ? '✓' : '–'), center: true, done: (r) => r.khuon_done },
  { h: 'Mực', w: 8, key: (r) => (r.muc_done ? '✓' : '–'), center: true, done: (r) => r.muc_done },
  { h: 'Trạng thái KT', w: 16, key: (r) => ((r.n_tech_done || 0) >= 3 ? 'Đủ 3 mục' : `${r.n_tech_done || 0}/3 mục`), center: true },
...
export default async function exportReadyQcExcel(rows, fileName = 'qc-ready') {
  ...
  rows.forEach((r, i) => {
    ...
    row.eachCell({ includeEmpty: true }, (cell, col) => {
      ...
      if (c.h === 'Trạng thái KT') cell.font = { color: { argb: (r.n_tech_done || 0) >= 3 ? 'FF16A34A' : 'FFB45309' }, bold: true };
      if (c.han) {
