...
export default function ReadyQcPage() {
  ...
  const columns = [
    ...
    { key: 'actions', header: '', className: 'text-right', render: (r) =>
      canQC && <Button className="px-3 py-1.5" onClick={() => open(r)}>QC xác nhận</Button> },
  ];
    ...
    <div>
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <Button onClick={doConfirm} loading={saving} disabled={!canQC || loadingDetail}>
              QC xác nhận
            </Button>
          </>
        }
      >
        <div className="mb-3 rounded-control bg-surface-muted px-3 py-2 text-sm text-ink-soft">
          {editing?.ten_khach_hang} · {editing?.ma_don_hang} · {editing?.ma_hang} · {editing?.mau_vai}
        </div>
        {loadingDetail ? (
          ...
          <div className="space-y-2">
            ...
            {TECH_ITEMS.map((it) => {
              ...
              return (
                <div key={it.ma} className="flex items-center justify-between rounded-control border border-line px-3 py-2">
