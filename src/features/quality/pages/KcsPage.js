...
export default function KcsPage() {
  ...
  const save = async () => {
    ...
    try {
      ...
      show(`KCS ${editing.ma_tem}: Đạt ${fmtNum(d.so_luong_dat)} → OQC · Sửa ${fmtNum(d.so_luong_sua)}`
  ...
  // Nhập "Số lượng hư" → kế thừa sang "Quyết định sửa" (vẫn sửa lại được, không đổi ngược lại số lượng hư).
  const onChangeHu = (e) => {
    ...
    <div>
      ...
      >
        ...
        <p className="text-xs text-ink-soft">
          Kiểm <b>từng phần</b> (đạt + hư + mẫu ≤ còn cần kiểm). Phần đạt → chờ <b>OQC</b>; phần sửa → <b>Sửa</b> (sửa xong lại qua OQC). Phần chưa kiểm giữ lại để kiểm lần sau.
        </p>
      </SidePanel>
