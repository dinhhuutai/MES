...
const empty = { maCheckpoint: '', tenCheckpoint: '', loaiCheckpointId: '', batBuoc: false, thuTu: '', cauHinhJson: '', moTa: '' };
...
export default function CheckpointPanel({ tram, onClose }) {
  ...
  const save = async () => {
    setSaving(true);
    try {
      const body = { ...form, tramId: tram.id, thuTu: form.thuTu === '' ? null : Number(form.thuTu) };
      if (editing) { await updateCheckpoint(editing.id, body); show('Đã cập nhật checkpoint'); }
      else { await createCheckpoint(body); show('Đã tạo checkpoint'); }
      setOpen(false); load();
  ...
  return (
    <SidePanel open onClose={onClose} title={`Checkpoint — ${tram.ma_tram}`} subtitle={tram.ten_tram} width="max-w-xl"
      footer={canManage && <Button icon="settings" onClick={openCreate}>Thêm checkpoint</Button>}>
      {loading ? (
        <div className="py-10 text-center text-ink-soft">Đang tải...</div>
      ) : (
        <div className="space-y-2">
          {rows.length === 0 && <p className="text-sm text-ink-soft">Chưa có checkpoint.</p>}
          {rows.map((c) => (
